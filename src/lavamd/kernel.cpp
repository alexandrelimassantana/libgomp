...
static void lava(box_str* box, int *npoints, int nboxes, FOUR_VECTOR* rv, double* qv, FOUR_VECTOR* fv, int nthreads, bool use_mogslib, int nchunks)
{
	...
	/* Data allocation. */
	std::vector<unsigned> _tasks(nboxes);
	...
	if(use_mogslib) {
		...
		std::get<0>(MOGSLib::API::contexts).set_loads(_tasks);
	} else {
		auto loopid = omp_loop_register("mainloop");
		omp_set_schedule(omp_sched_binlpt, nchunks);
		omp_set_workload(loopid, _tasks.data(), _tasks.size(), true);
	}
...
void kernel(int *tasks, int ntasks, int nthreads, bool use_mogslib, int nchunks)
{
	box_str *boxes;
	FOUR_VECTOR *rv;
	double *qv;
	...
	for (int i = 0; i < NITERATIONS; i++)
		lava(boxes, tasks, nboxes, rv, qv, fv, nthreads, use_mogslib, nchunks);
