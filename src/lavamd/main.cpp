...
extern void kernel(int *, int, int, bool, int);
...
static struct
{
	...
	int mogslib;			/**< Wether or not to use MOGSLib.*/
	int nchunks;			/**< Superior limit on how many chunks to produce.*/
} args = { NULL, 0, 0, 0, 0};
...
static void usage(void)
{
	...
	printf("  --mogslib <0/1>    Wether or not to use MOGSLib instead of native BinLPT\n");
	printf("  --nchunks <number>    The superior limit on how many chunks to produce\n");
...
static int *readfile(const char *input)
{
	...
	tasks = (int*) smalloc(args.ntasks*sizeof(int));
...
int main(int argc, const char **argv)
{
	...
	kernel(tasks, args.ntasks, args.nthreads, args.mogslib!=0, args.nchunks);
